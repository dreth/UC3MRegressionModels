...
\CommentTok{#> Fixed term is "(Intercept)"}
...
\NormalTok{trm1 <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(y }\OperatorTok{~}\StringTok{ }\NormalTok{x1 }\OperatorTok{+}\StringTok{ }\KeywordTok{sqrt}\NormalTok{(x2}\OperatorTok{+}\DecValTok{1}\NormalTok{) }\OperatorTok{+}\StringTok{ }\KeywordTok{minmax_scaler}\NormalTok{(}\KeywordTok{sqrt}\NormalTok{(x3)), }\DataTypeTok{data=}\NormalTok{transform)}
\KeywordTok{plot}\NormalTok{(trm1)}
\end{Highlighting}
\end{Shaded}

\includegraphics{./figures/unnamed-chunk-16-1.pdf}
\includegraphics{./figures/unnamed-chunk-16-2.pdf}
\includegraphics{./figures/unnamed-chunk-16-3.pdf}
\includegraphics{./figures/unnamed-chunk-16-4.pdf}
