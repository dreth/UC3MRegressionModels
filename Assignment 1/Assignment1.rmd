...
for (j in 1:1000) {
    ...
    cos_t = c()
    for (i in 1:100) {
        eps = c(eps, rnorm(1, mean=0, sd=1))
        cos_t = c(cos_t, cos(i/10 + phi))
        run = beta1 + beta2*cos_t[i] + eps[i]
        vals = c(vals, run)
    }
    sim[[j]] = vals
    epsilon[[j]] = eps
    cosine_term[[j]] = cos_t
    beta1_est[[j]] = mean(sim[[j]]) - beta2*mean(cosine_term[[j]]) - mean(epsilon[[j]])
    beta2_est[[j]] = (mean(sim[[j]]) - beta1 - mean(epsilon[[j]]))/mean(cosine_term[[j]])
}
```
