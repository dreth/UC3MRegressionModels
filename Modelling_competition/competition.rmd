...
for (i in 1:length(categorical)) {
    credit$categorical[i] <- as.factor(credit$categorical[i])
}
names(credit)
```
...
forms <- model_formula(Train, 2, "response", with_int=                 formulas_without        pvals
144            response~furniture+male_single 0.9958392781
28                response~chk_acct+telephone 0.9952971148
106              response~new_car+num_credits 0.9921654502
346    response~co.applicant+present_resident 0.9883530920
4                  response~chk_acct+used_car 0.9849181807TRUE, all=2)
```
...
test <- function(formulas_with, formulas_without, models_with_int, models_without_int) {
    ...
    for (i in 1:length(formulas_with)) {
        ...
        print(anova(models_with_int[[i]], models_without_int[[i]], test="Chisq"))
    }
...
formulas_without <- model_formula(Train, 2, "response", with_int=FALSE, all=2)
...
```{r, echo=TRUE, warning=FALSE, message=FALSE}
cool_stuff[order(-cool_stuff$pvals),]
```
